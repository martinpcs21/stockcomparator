...
def get_company_data(ticker):
    ...
    for i, l in enumerate(ls):
        if l.startswith('"annual') and ls[i + 1].startswith('"reportedValue"') and ls[i + 2].startswith(
                '"reportedValue"') and ls[i + 3].startswith('"reportedValue"') and ls[i + 4].startswith(
                '"reportedValue"'):
        ...
        elif l.startswith('"annual') and ls[i + 1].startswith('"reportedValue"') and ls[i + 2].startswith(
                '"reportedValue"') and ls[i + 3].startswith('"reportedValue"') and Columns == 4:
            CashFlowList.append(ls[i][:-17])
            CashFlowList.append(ls[i + 3].lstrip('"reportedValue":{"raw":'))
            CashFlowList.append(ls[i + 2].lstrip('"reportedValue":{"raw":'))
            CashFlowList.append(ls[i + 1].lstrip('"reportedValue":{"raw":'))
...
if __name__ == '__main__':
    ...
    for stockTicker in stockTickerList:
        try:
            get_company_data(ticker=stockTicker)
        except:
            print('Skipped data for ', stockTicker)
