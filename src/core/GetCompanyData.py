...
def get_company_data(ticker_name):
    def df_to_database(ticker_name, years, df):
        df = df.dropna()
    ...
    dates = cash_flow.loc['asOfDate'].values.tolist()
    years = []
    report_dates = []
    ...
    income_statement_db = df_to_database(ticker_name, years, income_statement)
    income_statement_db['KPI'] = 'annual' + income_statement_db['KPI']

    cash_flow_db = df_to_database(ticker_name, years, cash_flow)
    cash_flow_db['KPI'] = 'annual' + cash_flow_db['KPI']

    balance_sheet_db = df_to_database(ticker_name, years, balance_sheet)
    ...
    """for i in len(range(report_dates[1::]):
        year_prices=prices[prices['date'].dt.year == year]
        year_mean_price.append(year_prices['open'].mean())
        year_max_price.append(year_prices['high'].max())
        year_min_price.append(year_prices['low'].min())"""
