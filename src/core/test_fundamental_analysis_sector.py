...
if CompanyVariables['Sector'] == 'Financial Services':
    if Fundamentals.loc['Price to Book Value at Year Reference Price'].mean(axis=0) < 0.6:
        exit_multiple = 0.7
        DCFExitValue = exit_multiple * (Fundamentals.loc['Price to Book Value at Year Reference Price'].mean(axis=0)) * \
                       AnnualEquity.iloc[0]
    else:
        exit_multiple = 0.7
...
growth_score = ( \
                           ...
                           0.15 * np.maximum(-0.25,
                                             np.minimum(0.25, Fundamentals.loc['Free Cash Flow tendency'].iloc[0])) + \
                           0.15 * np.maximum(-0.25, np.minimum(0.25,
                                                               Fundamentals.loc['Operating Cash Flow tendency'].iloc[
                                                                   0])) + \
                           0.15 * np.maximum(-0.25, np.minimum(0.25, Fundamentals.loc['Net income tendency'].iloc[0])) + \
...
earnigns_score = \
    ...
    0.4 * (30 - np.maximum(0,
                           np.minimum(25, Fundamentals.loc['PFCF Price/ Free Cash Flow per Stock'].iloc[0] - 5))) / 25 + \
