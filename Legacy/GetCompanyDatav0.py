...
CashFlowDFDataBase=pd.DataFrame({
    'Company':CashFlowDataBaseStockNameList,
...
IncomeDFDataBase=pd.DataFrame({
    'Company':IncomeDataBaseStockNameList,
...
BalanceDFDataBase=pd.DataFrame({
    'Company':BalanceDataBaseStockNameList,
...
Balance_st.insert(0,'Company',StockName) #insert a column with the company name in the dataframe
...
StatisticsDFDataBase=pd.DataFrame({
    'Company':StatisticsDataBaseStockNameList,
...
try:
    sqliteConnection = sqlite3.connect('Fundamentalsv2.db')
    cur = sqliteConnection.cursor()
    print("Database created and Successfully Connected to SQLite")
